...
void wrapped_vadd(int *v1, int *v2, int *vo, int size, unsigned int polling_rate)
{
    ...
    std::cout << "Replacing the buffer and kernel info in the async call\n";
    replace_async_buf(putchar0, host_ptr_putchar0_buf, host_ptr_putchar0_info);
    replace_async_buf(assert0, host_ptr_assert0_buf, host_ptr_assert0_info);
