...
void wrapped_vadd(int *v1, int *v2, int *vo, int size)
{
    ...
    auto putchar0 = create_async_buf("c", 1);
    ...
    auto bo_putchar0 = xrt::bo(device, putchar0->size, kernel.group_id(0));

    int *host_ptr_v1 = bo_v1.map<int *>();
    int *host_ptr_v2 = bo_v2.map<int *>();
    int *host_ptr_vo = bo_vo.map<int *>();
    async_call_buf *host_ptr_putchar0 = bo_putchar0.map<async_call_buf *>();
